...
for card in cards:
    ...
    preco = preco_elem.get_text(strip=True).replace('R$', '') if preco_elem else "Sem preço"
...
# Loop para acessar as próximas páginas
while True:
    # Fazer requisição
    r = requests.get(url, headers=headers)
    if r.status_code != 200:
    ...
    for card in cards:
        # Buscar título
        titulo_elem = card.find('a', class_='ui-search-link__title-card ui-search-link')
        titulo = titulo_elem.get_text(strip=True) if titulo_elem else "Sem título"

        # Buscar preço (procurando diretamente pelo valor)
        preco_elem = card.find('span', class_='andes-money-amount ui-search-price__part ui-search-price__part--medium andes-money-amount--cents-superscript')
        if not preco_elem:
            preco_elem = card.find('span', 'andes-money-amount__fraction')
        preco = preco_elem.get_text(strip=True).replace('R$', '') if preco_elem else "Sem preço"
